...
bool TxHdl_FrameProc(void *frame, bool apFrame, u32 priority, u32 flags)
{
    ...
    if ((padding!=0)&&(host_txreq->f80211==0)){
        ...
        MEMCPY((void *)tbuf,(void *)OS_FRAME_GET_DATA(frame),copy_len);
        os_frame_pull(frame,padding);
        MEMCPY((void *)((u32)OS_FRAME_GET_DATA(frame)),tbuf,copy_len);

        host_txreq=(struct cfg_host_txreq0 *)OS_FRAME_GET_DATA(frame);
    }
