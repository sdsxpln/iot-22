...
{
	{ "sdio_simrxisr",   (OsMsgQ)0, 0, 	OS_RX_ISR_PRIO, 64, NULL,  sdio_sim_rx_isr},
};
#endif
...
s32 sdio_start(void)
{
  ...
  /*
    ...
    */
    ...
    sdio_write_reg(ADR_SDIO_MASK, 0xfffe1fff);    

    //Setting Tx resource low ---ID[0x5] Page[0x15]    
    sdio_write_reg(ADR_TX_LIMIT_INTR, 0x80000000 | 
...
static bool _sdio_do_firmware_checksum(u32 origin)
{
    ...
    do{
		...
		do{
            fw_status = sdio_read_reg(FW_STATUS_REG);
...
bool sdio_download_firmware(u8* fw_bin, u32 fw_bin_len)
{
    bool ret = TRUE;
//    u32 fw_status;
