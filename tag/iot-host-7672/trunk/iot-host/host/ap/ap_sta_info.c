...
extern struct ap_tx_desp *ap_tx_desp_peek(const struct ap_tx_desp_head *list_);
...
extern void purge_all_ps_buffers(APStaInfo_st *pAPStaInfo);
...
extern u32 ap_tx_desp_queue_len(const struct ap_tx_desp_head *list_);
...
//		unsigned long tids;
		...
		indicate_tim |= (ap_tx_desp_queue_len(&sta->ps_tx_buf[ac])!=0);
		if(indicate_tim)
			break;
...
static bool sta_info_buffer_expired(APStaInfo_st *sta, struct ap_tx_desp *tx_desp)
{
	int timeout;
	...
	return time_after((unsigned long)os_sys_jiffies(), (unsigned long)tx_desp->jiffies + timeout);
}
...
static bool sta_info_cleanup_expire_buffered_ac(APStaInfo_st *sta, int ac)
{
	...
	for (;;) {
		...
		if (sta_info_buffer_expired(sta, tx_desp))
			tx_desp = ap_tx_desp_dequeue(&sta->ps_tx_buf[ac]);
		else
			tx_desp = NULL;
...
void sta_info_cleanup(void *data1, void *data2)
{
	...
	os_create_timer(AP_STA_INFO_CLEANUP_INTERVAL, sta_info_cleanup, NULL, NULL, (void*)MBOX_CMD_ENGINE);
}
...
void APStaInfo_Init()
{
	...
	size = sizeof(APStaInfo_st) * AP_MAX_STA;
	gDeviceInfo->StaConInfo = MALLOC(size);
...
//------------------------------------------------------------------------------
	...
	if (next_time) {
		os_create_timer(next_time, ap_handle_timer, pApInfo, sta, (void*)MBOX_CMD_ENGINE);
		return;																					//---------------------------------------->Return
	}
	...
	switch (sta->timeout_next) 
	{
		case STA_NULLFUNC:
			...
			os_create_timer(AP_DISASSOC_DELAY, ap_handle_timer,
								   gDeviceInfo->APInfo, sta, (void*)MBOX_CMD_ENGINE);
...
// 			ieee802_1x_free_station(sta);
			...
			os_create_timer(AP_DEAUTH_DELAY, ap_handle_timer, pApInfo, sta, (void*)MBOX_CMD_ENGINE);
...
APStaInfo_st *APStaInfo_add(ApInfo_st *pApInfo, const u8 *addr)
{
	...
	LOG_TRACE( "%s\r\n",__func__);
	if (pApInfo->num_sta >= AP_MAX_STA) {
		/* FIX: might try to remove some old STAs first? */
		LOG_DEBUG( "no more room for new STAs (%d/%d)\r\n",
			   pApInfo->num_sta, AP_MAX_STA);
		return NULL;
	}
	...
	os_create_timer(AP_MAX_INACTIVITY, ap_handle_timer, pApInfo, sta, (void*)MBOX_CMD_ENGINE);
...
#if OLDCMDENG_TRX    
	_ssv6xxx_send_msg_to_hcmd_eng(tx_desp->frame);
#else
    TxHdl_FrameProc(tx_desp->frame, false, 0, 0);
#endif
				  ...
				  enum frame_release_type reason)
{
	...
	//			else 
				{
					...
					while (n_frames > 0) {										//------------------------------------->
						...
						n_frames--;
						found = true;
						ap_tx_desp_queue_tail(&frames, tx_desp);
					}
				}
	// 
	// 			/*
	...
	// 		}
	// 
			if (!ap_tx_desp_queue_empty(&sta->ps_tx_buf[ac])) {
				more_data = true;
				break;
			}
	 	}
		...
		if (!driver_release_tids) {
			...
			while ((tx_desp = ap_tx_desp_dequeue(&frames))) {
				...
				if (more_data || !ap_tx_desp_queue_empty(&frames))
				...
				if (reason == FRAME_RELEASE_UAPSD &&
				     ap_tx_desp_queue_empty(&frames))//Only set in Data frame ??? non data frame is last in queue
				{
...
void APStaInfo_Release(void)
{
	...
	do
	{				
		APStaInfo_free(gDeviceInfo->APInfo, &gDeviceInfo->StaConInfo[index]);				
		index++;		
	}while(index<AP_MAX_STA);
...
extern void ap_print_queue_info(struct ap_tx_desp_head *head);
...
void APStaInfo_PrintStaInfo(void)
{
	...
	LOG_TRACE("===================%s==================\r\n",__func__);
	LOG_TRACE("PS STA:%d BUF BC:%d Jiffies:%d\r\n", gDeviceInfo->APInfo->num_sta_ps, ap_tx_desp_queue_len(&(gDeviceInfo->APInfo->ps_bc_buf)), os_sys_jiffies());
    ...
    {	
		...
		for(i=0;i<IEEE80211_NUM_ACS;i++)
		{
			...
			LOG_TRACE("BUF AC[%d]:%d \r\n", i, ap_tx_desp_queue_len(&StaInfo->ps_tx_buf[i]));
