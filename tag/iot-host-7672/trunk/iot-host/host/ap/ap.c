...
void AP_RxHandleEvent(struct cfg_host_event *pHostEvt)
{

	//LOG_TRACE( "Enter AP_RxHandleEvent pHostEvt->c_type %d\n", pHostEvt->c_type);
	
	switch (pHostEvt->h_event)
	{
			
		case SOC_EVT_SET_BSS_PARAM_RESP:

			LOG_TRACE("%s Start to send Beacon\n",__func__);
			//BeaconGen(NULL, NULL);

			//Notify host command AP is ready.
			//host_cmd_cb_handler(SSV6XXX_AP_ON, SSV6XXX_SUCCESS);
		...
		case SOC_EVT_NULL_DATA:
			{
				struct ap_rx_desp rx_desp;
				struct cfg_null_data_info *null_data_info = (struct cfg_null_data_info *)(pHostEvt+1);
		...
		case SOC_EVT_DTIM_EXPIRED:
		{
			LOG_TRACE("%s DTIM Expired\r\n",__func__);			
			ap_tx_dtimexpiry_event();			
		}
		break;
...
//----------------------
	//init broadcast list
	ap_tx_desp_head_init(&gDeviceInfo->APInfo->ps_bc_buf);
...
//	LOG_TRACE( "Enter AP_Init \n");
	...
	//Belong to cmd engine
	{
		...
		gDeviceInfo->APInfo = MALLOC(size);
		ASSERT_RET(gDeviceInfo->APInfo != NULL, OS_FAILED);
		MEMSET(gDeviceInfo->APInfo, 0, size);
...
bool ssv6xxx_beacon_set(void* beacon_skb, int dtim_offset)
{
	...
	MAC_REG_WRITE( reg_tx_beacon_adr,  val);
