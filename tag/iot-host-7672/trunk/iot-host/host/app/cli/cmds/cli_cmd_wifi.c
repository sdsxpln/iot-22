...
struct ssv6xxx_ieee80211_bss * wifi_find_ap_ssid(char *ssid)
{
#if (MLME_SETTING == 0)
    int i = 0;
    ...
    for (i = 0; i < NUM_AP_INFO; i++) 
    {
        if ((sg_ap_info[i].ap_info != NULL) && 
            (strcmp((const char *) sg_ap_info[i].ap_info->ssid.ssid, ssid) == 0))
        {
            break;
        }
    }
...
void _join_result_handler (void *data)
{
    ...
    netmgr_set_netif_status(LINK_UP);
...
void _cmd_wifi_join (s32 argc, s8 *argv[])
{
    ...
    if (argc == 1)
    {
        ret = netmgr_wifi_join_async(argv[0], WIFI_SEC_NONE, NULL);
    } 
    else if (strcmp(argv[1], "wep40") == 0)
    {
        ret = netmgr_wifi_join_async(argv[0], WIFI_SEC_WEP_40, (argc == 3) ? argv[2]: NULL);
	} 
    else if (strcmp(argv[1], "wep104") == 0)
	{
        ret = netmgr_wifi_join_async(argv[0], WIFI_SEC_WEP_104, (argc == 3) ? argv[2]: NULL);
    }
    else if (strcmp(argv[1], "wpa") == 0)
    {
        ret = netmgr_wifi_join_async(argv[0], WIFI_SEC_WPA_PSK, (argc == 3) ? argv[2]: NULL);
    } 
    else if (strcmp(argv[1], "wpa2") == 0)
    {
        ret = netmgr_wifi_join_async(argv[0], WIFI_SEC_WPA2_PSK, (argc == 3) ? argv[2]: NULL);
    }
    else 
    {
        ret = netmgr_wifi_join_async(argv[0], WIFI_SEC_NONE, NULL);
...
void cmd_ctl(s32 argc, s8 *argv[])
{
	...
	else if (strcmp(argv[1], "status")==0)
    {
        ...
        {
            LOG_PRINTF("Mode:Station, %s\n",(info.u.station.apinfo.status == CONNECT) ? "connected" :"disconnected");
