...
static s32 wsimp_snd_cmd(s8 *dat, s32 len)
{
    HDR_HostCmd *HostCmd;
...
s32 wsimp_mac_str2hex(s8 *str, ETHER_ADDR *mac)
{
...
FILE *wsimp_fopen(s8 *file, s8 *mode)
{
    FILE *fs;
...
s32 wsimp_tokenlize(s8 *buff, s8 *argv[])
{
    s32 argc;
    s8 ch, *pch;
...
s32 wsimp_generic_extrainfo(struct wsimp_file_info *wf_info, void *args, s32 argc, s8 *argv[])
{
...
{
    s8 *argv[128], temp[36], *pos;
    s8 buffer[1024], buffer1[4096];
    s32 argc, i, state=0;
	s32 rd_count=0;
...
again:
        ...
        case 0:
            if (argc==2 && strcmp(argv[0], "PKT_ID:")==0 ) {
                ...
                wf_info->pkt_id = ssv6xxx_atoi((s8 *)argv[1]);
                state = 1;
            }
            else
            {
        ...
        case 1:
            ...
            if (argc==2 && strcmp(argv[0], "encap:")==0) {
                ...
                wf_info->priority = ssv6xxx_atoi((s8 *)argv[1]);
                wf_info->with_qos = 1;
                ...
                wf_info->with_qos = 1;
                wf_info->qos = ssv6xxx_atoi((s8 *)argv[1]);
                break;
            }
...
u32 wsimp_crc32(s8 *message, s32 len) 
{  
    s32 i, j;  
    u32 byte, crc;
    i = 0;  
