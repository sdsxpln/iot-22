...
static void
list_add( Timer* t, void *iperf_test )
{
    Timer* t2;
    Timer* t2prev;
...
static void
list_remove( Timer* t, void *iperf_test )
{
    struct iperf_test *test = (struct iperf_test *)iperf_test;
    if ( t->prev == NULL )
    	test->timers = t->next;
...
static void
list_resort( Timer* t, void *iperf_test )
{
    /* Remove the timer from the list. */
    list_remove( t, iperf_test );
    /* And add it back in, sorted correctly. */
    list_add( t, iperf_test );
}


static void
add_msecs( struct timeval* t, s32 msecs )
{
    t->tv_sec += msecs / 1000L;
    t->tv_usec += (msecs % 1000L)*1000;
