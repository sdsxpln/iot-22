...
{
	u16 nOffset = HCmdEng_GetRawRxDataOffset(rxpkt);
	return (((u8*)rxpkt)+nOffset);
}


//extern inline s32 _ssv6xxx_wifi_send_ethernet(void *frame, s32 len, enum ssv6xxx_data_priority priority, u8 tx_dscrp_flag,const bool mutexLock );
extern u16 HCmdEng_GetRawRxDataOffset(struct cfg_host_rxpkt *pPktInfo);
		...
		//Check if DA in our station list.
					(pStaInfo = APStaInfo_FindStaByAddr((ETHER_ADDR*)pEthHdr->h_dest)) )//ieee80211_deliver_skb		
				{
					...
					if (pStaInfo)				
					{
						...
						//LOG_TRACE("****************Relay unicast frame to sta " ETH_ADDR_FORMAT " .\n", ETH_ADDR(pEthHdr->h_dest));						 
					}
		 			else
		 			{
...
//						LOG_TRACE("****************Relay broadcast frame.\n");
		 			}
					...
					if(pRelayFrame)
					{
						os_frame_pull(pRelayFrame, HCmdEng_GetRawRxDataOffset((struct cfg_host_rxpkt *)OS_FRAME_GET_DATA(pRelayFrame)));
