...
void ieee802_11_set_beacon(ApInfo_st *pApInfo)
{
//--------------------------------
//Head-TIM-Tail
//--------------------------------
...
//***************************************************************************
	...
	tailpos = hostapd_eid_erp_info(pApInfo, tailpos);
 	...
 	/* RSN, MDIE, WPA */
 	tailpos = hostapd_eid_wpa(pApInfo, tailpos, AP_BEACON_TAIL_BUF_SIZE - (tailpos-(u8*)pApInfo->pBeaconTail) );
	...
	*pos++ = AP_DEFAULT_DTIM_PERIOD;
	...
	if (have_bits) {
		...
		*pos++ = n1 | aid0;
		/* Part Virt Bitmap */
		//skb_put(skb, n2 - n1);
		MEMCPY(pos, pApInfo->tim + n1, n2 - n1 + 1);
...
void StartBeacon(void)
{
	...
	ieee802_11_set_beacon(gDeviceInfo->APInfo);
}
