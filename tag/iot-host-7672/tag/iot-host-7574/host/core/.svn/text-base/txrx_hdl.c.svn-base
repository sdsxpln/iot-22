...
u16 _GetRawRxDataOffset(struct cfg_host_rxpkt *pPktInfo)
{
	u16 offset=0;
	...
	switch(gDeviceInfo->tx_type)
	{
		case SSV6XXX_TX_REQ0:
            ...
            if((((u32)OS_FRAME_GET_DATA(frame))&0x00000003) != 0)
            {
                ...
                MEMSET(host_txreq1, 0, sizeof(struct cfg_host_txreq0));
...
s32 RxHdl_FrameProc(void* frame)
{
    ...
    {
        u16 eth_type = 0;
        u8* eth_type_addr = (u8*) ((u8*)pPktInfo + (_GetRawRxDataOffset((struct cfg_host_rxpkt *)OS_FRAME_GET_DATA(frame)) + 12));
	...
	if (SSV6XXX_DATA_CONT == data_ret)
	{
		...
		os_frame_pull(frame, _GetRawRxDataOffset((struct cfg_host_rxpkt *)OS_FRAME_GET_DATA(frame)));
