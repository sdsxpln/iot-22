...
extern void os_timer_expired(struct os_timer *pOSTimer);
...
//-------------------------------------------------------------
extern void HCmdEng_RxHdlEvent(struct cfg_host_event *pHostEvt);
extern void HCmdEng_TxHdlCmd(struct cfg_host_cmd *pPktInfo);
#ifdef OLDCMDENG_TRX
extern void HCmdEng_RxHdlData(struct cfg_host_rxpkt *pPktInfo);
extern void HCmdEng_TxHdlData(struct cfg_host_txreq0 *pPktInfo);
#endif
...
#ifdef OLDCMDENG_TRX
	case M0_RXEVENT:
		HCmdEng_RxHdlData(frame);
		break;
#endif

	case HOST_EVENT:
		HCmdEng_RxHdlEvent(frame);
		break;			
		//-------------
		//TX
	case HOST_CMD:
    {
        PendingCmdQ *pcmd = NULL;
        struct cfg_host_cmd *hCmd = (struct cfg_host_cmd *)OS_FRAME_GET_DATA(frame);
        
        if (gHCmdEngInfo->blockcmd_in_q == true) 
        {        
            pcmd = (PendingCmdQ *)MALLOC(sizeof(PendingCmdQ));
            pcmd->cmd_frame = frame;
            OS_MutexLock(gHCmdEngInfo->CmdEng_mtx);
            list_q_qtail(&gHCmdEngInfo->pending_cmds, (struct list_q *)pcmd);
            ...
            HCmdEng_TxHdlCmd(frame);
		break;
    }
#ifdef OLDCMDENG_TRX
	case M0_TXREQ:
		HCmdEng_TxHdlData(frame);
		break;
#endif
...
void CmdEng_Task( void *args )
{
	...
	{				
        if ((gHCmdEngInfo->blockcmd_in_q == false) && (list_q_len_safe(&gHCmdEngInfo->pending_cmds, &(gHCmdEngInfo->CmdEng_mtx)) > 0))
        {
            ...
            OS_MutexLock(gHCmdEngInfo->CmdEng_mtx);
            pcmd = (PendingCmdQ *)list_q_deq(&gHCmdEngInfo->pending_cmds);            
            OS_MutexUnLock(gHCmdEngInfo->CmdEng_mtx);
            hCmd = (struct cfg_host_cmd *)OS_FRAME_GET_DATA(pcmd->cmd_frame);
            if (gHCmdEngInfo->debug != false)
                LOG_DEBUG("[CmdEng]: Pop pending cmd %d to execute\n", hCmd->cmd_seq_no);
            HCmdEng_TxHdlCmd(pcmd->cmd_frame);
            FREE(pcmd);
        }
        else
        {
            /* Wait Message: */
            res = msg_evt_fetch(MBOX_HCMD_ENGINE, &MsgEv);
            ...
            case MEVT_HOST_TIMER:
                //os_timer_expired((struct os_timer *)MsgEv);
                {
...
{
	{ "host_cmd",   (OsMsgQ)0, 16, 	OS_CMD_ENG_PRIO, 64, NULL, CmdEng_Task },
	{ "SSVDrv_Rx",  (OsMsgQ)0, 4, 	OS_RX_TASK_PRIO, 64, NULL, SSV6XXX_Drv_Rx_Task     },
	{ "SSVDrv_Tx",  (OsMsgQ)0, 16, 	OS_TX_TASK_PRIO, 64, NULL, SSV6XXX_Drv_Tx_Task     },
...
void CmdEng_FlushPendingCmds()
{
    ...
    {
        PendingCmdQ *pcmd = (PendingCmdQ *)list_q_deq(&gHCmdEngInfo->pending_cmds);
        os_frame_free(pcmd->cmd_frame);
        FREE(pcmd);
    }
