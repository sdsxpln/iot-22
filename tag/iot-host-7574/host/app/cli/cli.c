...
struct task_info_st g_sim_task_info[] = {
    { "cli_task",   NULL, 0, OS_CLI_PRIO, 64, (void *)1, Cli_Task},
...
void Cli_Init(s32 argc, s8 *argv[])
{
	...
	*/
    ...
    for(i=0; i<sizeof(g_sim_task_info)/sizeof(struct task_info_st); i++) {
        OS_TaskCreate(g_sim_task_info[i].task_func, 
        g_sim_task_info[i].task_name,
        g_sim_task_info[i].stack_size<<4,
        NULL, 
        g_sim_task_info[i].prio,
        NULL);
    }
