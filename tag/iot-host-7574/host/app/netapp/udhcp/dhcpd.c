...
void dhcpd_lease_show()
{
	...
	for (i = 0; i < server_config.max_leases; i++) {
		...
		{
            LOG_PRINTF("[%02x:%02x:%02x:%02x:%02x:%02x] --- [%d.%d.%d.%d]\r\n", g_leases[i].lease_mac[0], g_leases[i].lease_mac[1], g_leases[i].lease_mac[2],
                                g_leases[i].lease_mac[3], g_leases[i].lease_mac[4], g_leases[i].lease_mac[5],
                                 ip4_addr1_16(&g_leases[i].lease_nip),
                      ip4_addr2_16(&g_leases[i].lease_nip),
                      ip4_addr3_16(&g_leases[i].lease_nip),
                      ip4_addr4_16(&g_leases[i].lease_nip));
		}
	}
}
...
static struct task_info_st st_dhcpd_task[] = 
{
    { "DHCPD",  (OsMsgQ)0, 4, OS_DHCPD_TASK_PRIO, 64, NULL, udhcpd_start_task   },
};
...
void  udhcpd_start_task(void *arg)
{
    ...
    if (g_leases == NULL)
    {
        OS_MemFree((void *)dest_sin);
        UDHCP_DEBUG("OS_MemAlloc failed!\r\n");
        return;
    }

    OS_MemSET((char *)g_leases, 0, sizeof(server_config.max_leases * sizeof(g_leases[0])));
