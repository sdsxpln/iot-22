...
// #endif /* CONFIG_IEEE80211R */
	 ...
	 * is used */
	resp = check_assoc_ies(pApInfo, sta, pos, left, reassoc);
...
fail:
	send_assoc_resp(pApInfo, sta, resp, reassoc, pos, left);
}
...
{
	handle_assoc(pApInfo, mgmt, len, 0);		
	return AP_MLME_OK;
}
...
#pragma message("===================================================")
	/* RSN, MDIE, WPA */
	pos = hostapd_eid_wpa(gDeviceInfo->APInfo, pos, epos - pos);
	...
	ap_soc_data_send(gDeviceInfo->APInfo->pOSMgmtframe, pos-gDeviceInfo->APInfo->pMgmtPkt, TRUE, 0);
...
static s32 ap_mlme_rx_disassoc(struct ApInfo*pApInfo, struct ieee80211_mgmt *mgmt, u16 len)
{
	...
	if (sta->timeout_next == STA_NULLFUNC ||
		...
		os_create_timer(AP_DEAUTH_DELAY, ap_handle_timer,
				       pApInfo, sta, (void*)MBOX_HCMD_ENGINE);
	}
...
//	int vlan_id = 0;
	...
	if (!(
		((pApInfo->auth_algs & WPA_AUTH_ALG_OPEN) &&
...
//#endif /* CONFIG_IEEE80211R */
	      ((pApInfo->auth_algs & WPA_AUTH_ALG_SHARED) &&
...
void AP_RxHandleAPMode(void *frame)
{
	...
	u32 nDataLen = pPktInfo->len - (raw-(u8*)pPktInfo);
