...
s32 scan_done_host_event_handler_(struct cfg_host_event *pPktInfo)
{
    ...
    if(scan_done->u.scan_done.result_code==0){
        for(i=0;i<MAX_CHANNEL_NUM;i++){
            gDeviceInfo->channel_edcca_count[i]+=scan_done->u.scan_done.channel_edcca_count[i];
            gDeviceInfo->channel_packet_count[i]+=scan_done->u.scan_done.channel_packet_count[i];
            //LOG_PRINTF("Channel %d: edcca count is %d, packet count is %d \r\n",i,scan_done->u.scan_done.channel_edcca_count[i],scan_done->u.scan_done.channel_packet_count[i]);
        }
