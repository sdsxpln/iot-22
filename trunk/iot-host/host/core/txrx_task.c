...
extern void _dq_status_handler (void *data);
...
s32 TXRXTask_RxFrameProc(void* frame)
{
    ...
    if(rxpkt->c_type == M0_RXEVENT)
    {
        ...
        {
            os_frame_free(frame);
            LOG_DEBUGF(LOG_TXRX|LWIP_DBG_LEVEL_WARNING,("[TXRXTask]: free rx pkt due to mode = stop\r\n"));
    ...
    else if(rxpkt->c_type == HOST_EVENT)
    {
        ...
        else if(pPktInfo->h_event ==SOC_EVT_DATA_QUEUE_STATUS)
        {
    ...
    {
        LOG_DEBUGF(LOG_TXRX|LWIP_DBG_LEVEL_WARNING,("[TXRXTask]: unavailable type of rx pkt\r\n"));
    ...
    while(curr_mode != MT_EXIT)
    {
        ...
        for(retry=0;retry<32;retry++)
        {
            if(rFrame == NULL)
            {
                ...
                {
                    OS_MsDelay(10);
                   	LOG_DEBUGF(LOG_TXRX|LOG_LEVEL_SERIOUS, ("[TxRxTask]: wakeup from sleep!\r\n"));
                    //continue;
                }
                os_frame_push(rFrame, DRV_TRX_HDR_LEN);
                msg_data = OS_FRAME_GET_DATA(rFrame);
            ...
            else
            {
                ...
                {
                    u32 sysint,swdg,chip_id0,chip_id2;
                    MAC_REG_READ(ADR_SYS_INT_FOR_HOST,sysint);
                    MAC_REG_READ(ADR_SYS_WDOG_REG,swdg);
                    MAC_REG_READ(ADR_CHIP_ID_0,chip_id0);
                    MAC_REG_READ(ADR_CHIP_ID_2,chip_id2);
                    //LOG_PRINTF("reg:%08x, reg1:%08x\r\n",reg,reg1);
                    if ((sysint&0x1)&&(swdg==0)&&
                        (chip_id0&0x31333131)&&(chip_id2&0x32303041)) // watchdog wack up & reset
                    {
						u32 fw_status = 0;
						MAC_REG_READ(FW_STATUS_REG,fw_status);

						//LOG_PRINTF("***********************************************g_hw_enable:%d fw_status:%08x\r\n", g_hw_enable, fw_status);
						if(fw_status == 0x5A5AA5A5 ||
							g_hw_enable == false){
							break;
						}
...
s32 TXRXTask_FrameEnqueue(void* frame, u32 priority)
{
    ...
    else
    {
        ...
        if(dataFrm == NULL)
        {
            LOG_DEBUGF(LOG_TXRX|LOG_LEVEL_SERIOUS, ("[TxRx_task]: No memory for incoming frame\r\n"));
            OS_MutexUnLock(tx_mtx);
            return false;
        }
    }
