...
s32 RxHdl_FrameProc(void* frame)
{
    ...
    if (SSV6XXX_HWM_SCONFIG == gDeviceInfo->hw_mode){
        //Trap7Count++;
        //#define GET_SEQ(p) \
        //    (((*((u8 *)((u32)p+RXINFO_SIZE+23))<<8)|(*((u8 *)((u32)p+RXINFO_SIZE+22))<<0))&0xfff0)>>4
        //LOG_PRINTF("c_type=%d, f80211=%d, seq=0x%x,len=%d, channel=%d,\r\n",pPktInfo->c_type, pPktInfo->f80211,GET_SEQ(pPktInfo),pPktInfo->len-RXINFO_SIZE,pPktInfo->RSVD);
        if(pPktInfo->f80211==1){
            for (i=0;i<PRMOISCUOUS_CB_NUM;i++)
    		{
    			promiscuous_data_handler handler = gDeviceInfo->promiscuous_cb[i];
    			if (handler)
    			{
                    handler(pPktInfo->RSVD,(u8 *)((u32)pPktInfo+RXINFO_SIZE),pPktInfo->len-RXINFO_SIZE);
    			}
    		}
