...
void _host_event_handler(u32 evt_id, void *data, s32 len)
{
    ...
    case SOC_EVT_PS_WAKENED:
        if(g_cli_ps)
        {
            ...
            msg_evt_post(g_cli_task_info[0].qevt, msg_evt);
        }
        break;
    default:
...
void _dq_status_handler (void *data)
{
    ...
    APStaInfo_st *sta = &gDeviceInfo->StaConInfo[dqStatus->u.dq_status.wsid];
...
{    
    ipinfo info;
    struct cfg_ps_wow_request wowreq;
...
#if(CONFIG_USER_MODE_SMART_CONFIG==1)
extern u16 UserSconfigChannelMask;
#endif
//extern u32 Trap7Count;
void _cmd_wifi_sconfig (s32 argc, char *argv[])
{
    /**
...
#if(CONFIG_USER_MODE_SMART_CONFIG==1)
    UserSconfigChannelMask=channel;
    //Trap7Count=0;
    NETMGR_USER_SCONFIG(&op);
#else
    NETMGR_SCONFIG(channel);
#endif
...
void _cmd_wifi_list(s32 argc, char *argv[])
{
    ...
    ap_list = MALLOC(sizeof(struct ssv6xxx_ieee80211_bss)*NUM_AP_INFO);
    if(!ap_list)
    {
        LOG_PRINTF("malloc ap_list fail \r\n");
        return;
    }
    ssv6xxx_get_aplist_info((void *)ap_list);
    ...
    for (i=0; i<NUM_AP_INFO; i++)
    {
        ...
        if(ap_list[i].channel_id!= 0)
		{
            ...
            LOG_PRINTF("SSID: %s\t", ap_list[i].ssid.ssid);
...
void cmd_ctl(s32 argc, char *argv[])
{
    ...
    {
        u32 r,reg;
        u32 maxcouunt = (u32)strtol(argv[2],NULL,10);
        u32 testcount=1;
        ...
        for(testcount=1 ;testcount <= maxcouunt;testcount++)
        {
            ...
            LOG_PRINTF("testcount:%d, sleep:%d\r\n",testcount,r);
            r= r*1000;
            Sleep(r);
            MAC_REG_READ(0xc0000800,reg);
...
void cmd_rc(s32 argc, char *argv[])
{
    ...
    {
        u16 rate_mask = (u16)ssv6xxx_atoi(argv[2]);

        ssv6xxx_set_rate_mask(rate_mask);
        return;
    }
    else if (strcmp(argv[1], "param")==0&&argc == 4)
    {
        u16 pf = (u16)ssv6xxx_atoi(argv[2]);
        u8 sft = (u8)ssv6xxx_atoi(argv[3]);

        ssv6xxx_set_rc_param(pf, sft);
        return;
    }
    LOG_PRINTF("Invalid wifictl command. rc mask 0x0[FFF] or rc param PF[12-24] SFT[8-4]\r\n");
}
