...
#define LOOPBACK_TEST_DATA_LEN (MAX_RECV_BUF-HOST_CMD_HDR_LEN)
static void _gen_random(u8 *data, u32 len)
{
    u32 idx=0;
    u32 rtemp;
...
void bus_loopback(int argc, char **argv)
{
    ...
    pLoopBackOut=os_frame_alloc(LOOPBACK_TEST_DATA_LEN);
    ...
    pLoopBackIn=os_frame_alloc(LOOPBACK_TEST_DATA_LEN);
    if(pLoopBackIn==NULL){
    ...
    while(loop||(forever==TRUE)){
        ...
        _gen_random(OS_FRAME_GET_DATA(pLoopBackOut),OS_FRAME_GET_DATA_LEN(pLoopBackOut));
