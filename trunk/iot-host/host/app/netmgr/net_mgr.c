...
static bool g_sconfig_user_mode=FALSE;
...
static int _netmgr_wifi_switch(wifi_mode mode, wifi_ap_cfg *ap_cfg, wifi_sta_join_cfg *join_cfg, u16 scanning_channel_mask);
#if !NET_MGR_NO_SYS
...
int find_ip_in_arp_table(u8 * mac,ip_addr_t *ipaddr)
{
...
int netmgr_send_arp_unicast (u8 *dst_mac)
{
    ...
    else if(find_ip_in_arp_table(dst_mac,&ipaddr))
    {
        return etharp_unicast(dst_mac, &ipaddr);
    }
...
bool netmgr_wifi_check_mac(unsigned char * mac)
{
    ...
    if (info->operate == SSV6XXX_HWM_AP)
    {
         bRet = false;

         for(i = 0; i < info->u.ap.stanum; i++)
         {
            if (MEMCMP(mac, info->u.ap.stainfo[i].Mac, 6) == 0)
            {
                bRet = true;
                break;
            }
...
int netmgr_wifi_sconfig_async(u16 channel_mask,bool user_mode)
{
    #if !NET_MGR_NO_SYS
    MsgEvent *msg_evt = NULL;
    ...
    msg_evt->MsgData2 = (u32)user_mode;
    msg_evt->MsgData3 = 0;

    msg_evt_post(st_netmgr_task[0].qevt, msg_evt);
    #else
    netmgr_wifi_sconfig(channel_mask,user_mode);
    #endif
    return 0;
}
...
int netmgr_wifi_sconfig(u16 channel_mask, bool user_mode)
{
...
static int _netmgr_wifi_switch(wifi_mode mode, wifi_ap_cfg *ap_cfg, wifi_sta_join_cfg *join_cfg, u16 scanning_channel_mask)
{
    ...
    u16  channel_mask=scanning_channel_mask;
    char *ssid[1];
    ...
    if (mode == SSV6XXX_HWM_STA)
    {
        ...
        if (join_cfg)
        {
            ...
            netmgr_wifi_scan(channel_mask, ssid, 1);
        }
...
{
    return _netmgr_wifi_switch(mode, ap_cfg, join_cfg, g_sta_channel_mask);
}

int netmgr_wifi_switch_to_sta(wifi_sta_join_cfg *join_cfg, u8 join_channel)
{
    return _netmgr_wifi_switch(SSV6XXX_HWM_STA, NULL, join_cfg, (1<<join_channel));
}
    ...
    switch (evt_id)
    {
        case SOC_EVT_SCAN_DONE:
        {
            ...
            if (scan_done)
            {
                ...
                scan_done_cpy = (struct resp_evt_result *)MALLOC(sizeof(struct resp_evt_result));
        ...
        case SOC_EVT_SCAN_RESULT: // join result
        {
            ...
            if (scan_res)
            {
                ...
                scan_res_cpy = (ap_info_state *)MALLOC(sizeof(ap_info_state));
        ...
        {
            ssv6xxx_wifi_pwr_saving(0,FALSE);
            msg_evt = msg_evt_alloc();
            ASSERT(msg_evt);
            msg_evt->MsgType = MEVT_NET_MGR_EVENT;
...
void netmgr_task(void *arg)
{
    ...
    while(1)
    {
        ...
        if (res != OS_SUCCESS)
        {
            if((SSV6XXX_HWM_STA == (ssv6xxx_hw_mode)ssv6xxx_wifi_get_mode()) &&
               (BUS_IDLE_TIME > 0))
            {
                u32 time_stamp;

                //LOG_PRINTF("xxx%d\r\n",lastTRX_time);
                if(0xFFFFFFFF != lastTRX_time)
                {
                    time_stamp = os_tick2ms(OS_GetSysTick());
                    lastTRX_time = ssv6xxx_drv_get_TRX_time_stamp();
                    //LOG_PRINTF("%d,%d\r\n",time_stamp,lastTRX_time);
                    if((time_stamp>lastTRX_time)&&((time_stamp-lastTRX_time)>BUS_IDLE_TIME))
                    {
                        LOG_PRINTF("Bus idle time > %d\r\n",BUS_IDLE_TIME);
                        lastTRX_time = 0xFFFFFFFF;
                        ssv6xxx_wifi_pwr_saving(0,TRUE);
                    }
                }
            }
            else
            {
                //LOG_PRINTF("WIFI mode != STA = %d\r\n",ssv6xxx_wifi_get_mode());
            }
            continue;
        }
        else if (msg_evt && (msg_evt->MsgType == MEVT_NET_MGR_EVENT))
        {
            ...
            switch (msg_evt->MsgData)
            {
                ...
                case MSG_SCONFIG_REQ:
                {
                    ...
                    bool user_mode = msg_evt->MsgData2;
                    MsgEvent *msg_evt = NULL;
                ...
                case MSG_SCONFIG_DONE:
                {
                    ...
                    if(sconfig_done_cpy!=NULL){
                        netmgr_wifi_sconfig_done((u8 *)&sconfig_done_cpy->u.sconfig_done.rand,1,TRUE,10000);
                ...
                case MSG_SCONFIG_SCANNING_DONE:
                {
                    ...
                    netmgr_wifi_switch_to_sta(join_cfg,sconfig_done_cpy->u.sconfig_done.channel);
                ...
                {
                    if(FALSE==netmgr_wifi_check_sconfig()){
                        g_sconfig_user_mode=FALSE;
                    }else{
                        MsgEvent *msg_evt = NULL;
