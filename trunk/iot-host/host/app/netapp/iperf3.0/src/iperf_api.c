...
iperf_print_results(struct iperf_test *test)
{
        ...
        SLIST_FOREACH(sp, &test->streams, streams) {
        	...
        	if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                ...
                if (sp->outoforder_packets > 0 )
                    iprintf(test, iperf_report_sum_outoforder, sp->socket, start_time, end_time, sp->outoforder_packets);
                }
    ...
    if (test->num_streams > 1) {
        ...
        if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            ...
            iprintf(test, iperf_report_sum_bw_udp_format_send, start_time, end_time, ubuf, nbuf, iperf_report_sender);
            ...
            iprintf(test, iperf_report_sum_bw_udp_format, start_time, end_time, ubuf, nbuf, avg_jitter, lost_packets, total_packets, lost_percent, iperf_report_receiver);
        }
    }
