...
//#endif /* CONFIG_IEEE80211R */
    ...
    //findout timeout station
    if(!sta)
    {
        s32 idx=0,del_idx=WLAN_MAX_STA;
        u8 max_arp_retry_count=0;
        APStaInfo_st *StaInfo;
        //find max timeout count
        for(idx=0; idx<WLAN_MAX_STA; idx++)
        {
             StaInfo= &gDeviceInfo->StaConInfo[idx];
             if(StaInfo->arp_retry_count>max_arp_retry_count 
                && StaInfo->arp_retry_count>STA_TIMEOUT_RETRY_COUNT) //arp_retry_count>0, means no response over 15 sec
             {
                del_idx = idx;
        ...
        if(del_idx<WLAN_MAX_STA)
        {
            //LOG_PRINTF("del_idx:%d\r\n",del_idx);
            StaInfo= &gDeviceInfo->StaConInfo[del_idx];
