...
void APStaInfo_Init()
{
	...
	gDeviceInfo->StaConInfo = (struct APStaInfo *)MALLOC((u32)size);
	...
	for (j=0;j<gMaxAID;j++)
    {
        ...
        gDeviceInfo->StaConInfo[j].wsid_idx = WLAN_MAX_STA;
...
void APStaInfo_PrintStaInfo(void)
{
    ...
    for(idx=0; idx<gMaxAID; idx++)
    {
        ...
        if(StaInfo->arp_retry_count>STA_TIMEOUT_RETRY_COUNT)
            LOG_PRINTF(" Inactive\r\n");
        else
            LOG_PRINTF(" Active\r\n");

        //LOG_PRINTF("=>addr:"MACSTR" aid:%d _flag:%08x", MAC2STR(StaInfo->addr), StaInfo->aid, StaInfo->_flags);
        LOG_PRINTF("addr:"MACSTR"\r\n", MAC2STR(StaInfo->addr));
        LOG_PRINTF("aid:%d \r\n", StaInfo->aid);
        ...
        if(is_valid_ether_addr(StaInfo->addr))
        {
            LOG_PRINTF("RSSI = -%d (dBm)\r\n",ssv6xxx_get_rssi_by_mac(StaInfo->addr));
        }
