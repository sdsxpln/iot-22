...
int ssv6xxx_do_iq_calib(struct ssv6xxx_iqk_cfg *p_cfg)
{
    ...
    u32 fw_status=0;
    if(gDeviceInfo->reload_fw_cnt < MAX_RECOVER_COUNT)
    ...
    //return ;
    // make command packet
    frame = (void*)OS_MemAlloc(HOST_CMD_HDR_LEN + IQK_CFG_LEN + PHY_SETTING_SIZE + RF_SETTING_SIZE);
...
#endif
    p_cfg->cfg_pabias_ctrl = 0;
    p_cfg->cfg_pacascode_ctrl = 0;
    ...
    else if(chip_sel->chip_id == SSV6060P)
    {
        p_cfg->cfg_def_tx_scale_11b=(wifi_tx_gain[2]>>0) & 0xff;
        p_cfg->cfg_def_tx_scale_11b_p0d5=(wifi_tx_gain[2]>>8) & 0xff;
        p_cfg->cfg_def_tx_scale_11g=(wifi_tx_gain[0]>>16) & 0xff;
        p_cfg->cfg_def_tx_scale_11g_p0d5=(wifi_tx_gain[0]>>24) & 0xff;
    }
    else
    {
    ...
    if(gDeviceInfo->reload_fw_cnt < MAX_RECOVER_COUNT)
        ...
        p_cfg->cfg_def_tx_scale_11b,
        p_cfg->cfg_def_tx_scale_11b_p0d5,
        p_cfg->cfg_def_tx_scale_11g,
        p_cfg->cfg_def_tx_scale_11g_p0d5);
...
//Check efuse chip id. Must be CHIP_ID_SSV6030P
void check_efuse_chip_id(void)
{
	u32 efuse_chip_id;
	efuse_chip_id = read_chip_id();        
	if(efuse_chip_id != CHIP_ID_SSV6030P && efuse_chip_id != 0)
   	{      
        LOG_PRINTF("\33[31m[Error] Wi-fi CHIP ID mismatch iComm 6030!\33[0m\r\n");
        while(1);
    }
}
...
int cabrio_init_mac()
{
    ...
    *((u32 *)&chip_id[12]) = (u32)LONGSWAP(regval);
...
#if(CONFIG_USER_MODE_SMART_CONFIG==1)
    ...
    regval|=(RXINFO_SIZE<<RX_INFO_SIZE_SFT);
...
*/
    ...
    //Load FW
    {
        extern void ssv6xxx_download_firmware(void);
        ssv6xxx_download_firmware();
    }
...
int cabrio_init_ap_mac()
{
    ...
    {
    const unsigned char cmd_data[] = {
        0x04, 0x01, 0x00, 0x00, 0x70, 0x00, 0x00, 0x00};
    _ssv6xxx_wifi_ioctl_Ext(SSV6XXX_HOST_CMD_SET_FCMD_RXCTRL, (void *)cmd_data, 8, TRUE, FALSE);
    ...
    //for WEP security setting
    {
        ...
        switch (gDeviceInfo->APInfo->sec_type)  {
            case SSV6XXX_SEC_WEP_40:
                ...
                sec_entry.keyLen = strlen((void*)gDeviceInfo->APInfo->password);
                sec_entry.keyIndex = 0;
                MEMCPY(sec_entry.key,  gDeviceInfo->APInfo->password, strlen((void*)gDeviceInfo->APInfo->password));
...
void ssv6xxx_watchdog_enable(ssv6xxx_hw_mode hw_mode,bool on)
{

    if(on && hw_mode == SSV6XXX_HWM_AP)
...
void ssv6xxx_HW_disable()
{
    ...
    ssv6xxx_watchdog_enable(gDeviceInfo->hw_mode,FALSE);
#endif //#if(ENABLE_WATCHDOG == 1)
...
void ssv6xxx_HW_enable(void)
{
    ...
    ssv6xxx_drv_irq_enable();
#if(ENABLE_WATCHDOG == 1)
    ssv6xxx_watchdog_enable(gDeviceInfo->hw_mode,TRUE);
#endif //#if(ENABLE_WATCHDOG == 1)
	g_hw_enable = TRUE;
    return;
